...
from htransform.losses import epsilon_based_loss_fn_finetuning, controlnet_loss
...
def controlnet_trainer(
    ...
    optimizer = Adam(controlnet.parameters(), lr=optim_kwargs["lr"])
    ...
    for epoch in tqdm(range(optim_kwargs["epochs"])):
        ...
        if (epoch % optim_kwargs["save_model_every_n_epoch"]) == 0 or (
            ...
            if epoch == optim_kwargs["epochs"] - 1:
                torch.save(
                    controlnet.state_dict(), os.path.join(log_dir, "model.pt")
                )
            else:
                torch.save(
            ...
            torch.save(
                controlnet.state_dict(), os.path.join(log_dir, "model_tmp.pt")
            )
            torch.save(
        ...
        if val_kwargs["sample_freq"] > 0:
            if epoch % val_kwargs["sample_freq"] == 0:
                ...
                def combined_model(x, t):
                    # print("t : ", t)
                    control = controlnet(x, t, cond)
                    eps = pretrained_model(x, t,control=control)
                ...
                def combined_model_ema(x, t):
                    ...
                    control = ema.ema_model(x, t, cond)
                    eps = pretrained_model(x, t,control=control)
    ...
    torch.save(controlnet.state_dict(), os.path.join(log_dir, "model.pt"))
    torch.save(ema.ema_model.state_dict(), os.path.join(log_dir, "ema_model.pt"))

    return controlnet, ema
