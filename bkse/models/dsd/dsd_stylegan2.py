...
class DSDStyleGAN2(DSD):
    ...
    def load_synthesis_network(self):
        ...
        self.synthesis.load_state_dict(
            torch.load("experiments/pretrained/stylegan2.pt")["g_ema"], strict=False
        )
    ...
    def initialize_mapping_network(self):
        if Path("experiments/pretrained/gaussian_fit_stylegan2.pt").exists():
            self.gaussian_fit = torch.load(
                "experiments/pretrained/gaussian_fit_stylegan2.pt"
            )
        else:
            if self.verbose:
    ...
    def get_gen_im(self, latent_in):
        return (
            self.synthesis([latent_in], input_is_latent=True, noise=self.noise)[0] + 1
        ) / 2
