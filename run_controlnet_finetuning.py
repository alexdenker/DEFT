...
from htransform.controlnet_trainer import controlnet_trainer
...
def coordinator(args):
    ...
    with wandb.init(**wandb_kwargs) as _:
        ...
        control_net.load_state_dict(torch.load(model_config.data.model_path), strict=False)
        control_net.to("cuda")
        ...
        print(
            "number of parameters in control_net model: ",
        ...
        log_dir = os.path.join(base_path, "control_net", run_id)
        ...
        finetuned_score, ema = controlnet_trainer(
            controlnet=control_net.to(args.device),
