...
class SDS_VAR(DDIM):
    ...
    def sample(self, x, y, ts, **kwargs):
        ...
        sigma = torch.autograd.Variable(
            var, requires_grad=True
        )  # , device=device).type(dtype)
        optimizer = torch.optim.Adam(
            [mu, sigma], lr=self.lr, betas=(0.9, 0.99), weight_decay=0.0
        )  # original: 0.999
        ...
        for ti, si in zip(reversed(ts), reversed(ss)):
            ...
            grad_sigma = (
                ...
                * (1 / snr_inv)
                * sigma
                * (noise_xt * (et - (1 / eta_t) * noise_xt)).sum()
            )
    ...
    def initialize(self, x, y, ts):
        return torch.randn_like(x), 0.1 * torch.randn(
            1
        ).abs().cuda()  # torch.zeros(1).cuda()
