...
class MCG(DDIM):
    ...
    def sample(self, x, y, ts, **kwargs):
        ...
        for ti, si in zip(reversed(ts), reversed(ss)):
            ...
            grad_term = (
                torch.autograd.grad(mat_x, xt, retain_graph=True)[0]
                * self.grad_term_weight
                * alpha_t.sqrt()
            )
            grad_term = grad_term.detach() * (1 - H.singulars().view(x0_pred.size()))

            xs2 = xs1 - grad_term
            ys = H.H_pinv(y_0).view(xt.size()) * alpha_s.sqrt() + (
                1 - alpha_s
            ).sqrt() * torch.randn_like(xt)
            # x0_pred = (x0_pred + grad_term).detach()

            xs = xs2 * (
                1 - H.singulars().view(x0_pred.size())
            ) + ys * H.singulars().view(xt.size())
            xs = xs.detach()
