...
class REDDIFF_PARALLEL(DDIM):
    ...
    def sample(self, x, y, ts, **kwargs):
        ...
        optimizer = torch.optim.Adam(
            [mu], lr=1e-1, betas=(0.9, 0.99), weight_decay=0.0
        )  # original beta2: 0.999, lr=1e-1
        ...
        for ti, si in zip(ts_par, ss_par):
            t = (
                torch.ones(n, 1).to(x.device).long()
                * torch.tensor(ti).to(x.device).long()
            )  # bs x num_dens
            s = (
                torch.ones(n, 1).to(x.device).long()
                * torch.tensor(si).to(x.device).long()
            )
            t = t.view(1, -1)[0]
            s = s.view(1, -1)[0]
            ...
            loss_noise = torch.mul(
                w_t * (et - noise_xt).detach(), x0_pred_repeat
            ).mean()
